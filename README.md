...
#### 矩阵分解
在推荐系统中，我们常常遇到的问题是这样的，我们有很多用户和物品，也有少部分用户对少部分物品的评价，我们希望预测目标用户对其他未评分物品的评分，进而将评分高的物品推荐给目标用户。本质上是使用机器学习来给稀疏矩阵进行插值。

##### SVD
奇异值分解就不用多说了。将用户物品对应的$m \times n$矩阵$M$进行SVD分解，取其中前K个比较大的特征值来进行降维，此时矩阵M可以分解为：
...
**存在的问题**
SVD分解要求矩阵是稠密的，也就是矩阵的所有位置不能有空白。为了解决这个问题，一般来说是对评分矩阵中的缺失值进行补全。例如，用全局平均值或者用用户物品平均值补全。

##### FuncSVD
FunkSVD是在传统SVD面临计算效率问题时提出来的，既然将一个矩阵做SVD分解成3个矩阵很耗时，同时还面临稀疏的问题，那么我们能不能避开稀疏问题，同时只分解成两个矩阵呢？也就是说，现在期望我们的矩阵$M$这样进行分解：$$M_{m \times n}=P_{m \times k}^TQ_{k \times n}$$
...
将上式分别对$p_i, q_j$求导我们得到:$$\frac{\partial J}{\partial p_i} = -2(m_{ij}-q_j^Tp_i)q_j + 2\lambda p_i$$$$\frac{\partial J}{\partial q_j} = -2(m_{ij}-q_j^Tp_i)p_i + 2\lambda q_j$$

则在梯度下降法迭代时，$p_i, q_j$的迭代公式为： $$p_i = p_i + \alpha((m_{ij}-q_j^Tp_i)q_j - \lambda p_i)$$$$q_j =q_j +\alpha((m_{ij}-q_j^Tp_i)p_i - \lambda q_j)$$
